...
pub use ad::LeidongAd;
pub use app_list::LeidongAppList;
pub use app_request::LeidongAppRequest;
pub use app_response::LeidongAppResponse;
pub use creative_specs::LeidongCreativeSpecs;
pub use device::LeidongDevice;
pub use geo::LeidongGeo;
pub use icon::LeidongIcon;
pub use image::LeidongImage;
pub use imp::LeidongImp;
pub use interaction::LeidongInteraction;
pub use native::LeidongNative;
pub use request::LeidongRequest;
pub use response::LeidongResponse;
pub use site::LeidongSite;
pub use size::LeidongSize;
pub use text::LeidongText;
pub use tracker::LeidongTracker;
pub use user::LeidongUser;
pub use video::LeidongVideo;

pub struct Leidong {

}

impl Client for Leidong {
    ...
    async fn request(request: &Request, connection: &Connection, pool: &HttpPool, cache: &Cache) -> Result<Response, ResultMessage> {
        ...
        let request_leidong = LeidongRequest {
            request_id: {
            ...
            site: {
                LeidongSite {
                    ...
                    creative_specs: {
                        ...
                        if assets.get_banner_size() > 0 {
                            creative_specs.push(LeidongCreativeSpecs {
                                support_creative_type: 1,
                                support_sizes: LeidongSize {
                                    w: {
        ...
        let response_leidong: LeidongResponse;

        let client = {
            let pool_leidong = pool.pool_leidong.clone();
            let pool_leidong = pool_leidong.read().unwrap();
            pool_leidong.clone()
        };
        let response_leidong_raw = client.post("http://apiv2.leidong.rayenergytech.com/showad/v1.0")
            .json(&request_leidong)
        ...
        match response_leidong_raw {
            Ok(response_leidong_raw) => {
                let status = response_leidong_raw.status();
                if status != 200 {
                    return Err(ResultMessage {
                        code: 992,
                        message: {
                            match response_leidong_raw.text().await {
                    ...
                    match response_leidong_raw.text().await {
                        Ok(text) => {
                            match serde_json::from_str::<LeidongResponse>(&text) {
                                Ok(json) => {
                                    if json.ad.is_none() {
                                        return Err(ResultMessage {
                                            code: 993,
        ...
        let response = Response {
            ...
            seatbid: {
                ...
                let mut bid = vec![];
                let ad = response_leidong.ad.unwrap();
                ...
                bid.push(Bid {
                    ...
                    burl: {
                        ...
                        match ad.native.ks_winnotice_tracker {
                            Some(ks_winnotice_tracker) => {
                                let nurl = ks_winnotice_tracker.clone();
                                burl.push(replace_macro(&nurl));
                            },
                            None => (),
                        }
                    ...
                    lurl: {
                        ...
                        match ad.native.ks_lnurl {
                            Some(ks_lnurl) => {
                                let nurl = ks_lnurl.clone();
                                lurl.push(replace_macro(&nurl));
                            },
                            None => (),
                        }

                        match ad.native.lnurl_list {
                            Some(lnurl_list) => {
                                for lnurl in lnurl_list {
                                    let nurl = lnurl.clone();
                                    lurl.push(replace_macro(&nurl));
                                }
                            },
    ...
    async fn bidding_notify_win(url: String, win_price: i32, _next_price: i32, iv: &String, connection: &Connection, pool: &HttpPool) {
        ...
        let client = {
            let pool_leidong_lock = pool.pool_leidong.clone();
            let pool_leidong = pool_leidong_lock.read().unwrap();
            pool_leidong.clone()
        };
...
fn replace_macro(orig: &String) -> String {
    ...
    replaced = replaced.replace("__END_TS__", "__EVENT_TIME_END__");
