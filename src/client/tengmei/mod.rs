...
pub use app::TengmeiApp;
pub use app_info::TengmeiAppInfo;
pub use bid::TengmeiBid;
pub use caid::TengmeiCaid;
pub use device::TengmeiDevice;
pub use geo::TengmeiGeo;
pub use hc::TengmeiHc;
pub use imp::TengmeiImp;
pub use links::TengmeiLinks;
pub use material::TengmeiMaterial;
pub use request::TengmeiRequest;
pub use response::TengmeiResponse;
pub use seat_bid::TengmeiSeatBid;
pub use tracking::TengmeiTracking;
pub use user::TengmeiUser;

pub struct Tengmei {

}

impl Client for Tengmei {

    async fn request(request: &Request, connection: &Connection, pool: &HttpPool, cache: &Cache) -> Result<Response, ResultMessage> {
        let slot_id = connection.client_tag_id.split("|").nth(0).unwrap().to_string();

        let request_id = cache.get_sequence();
        let mut assets = Assets::new(request);
        let identifiers = Identifiers::new(request);

        let request_tengmei = TengmeiRequest {
            id: {
            ...
            imp: {
                vec![TengmeiImp {
                    ...
                    ad_type: {
                        ...
                        } else if assets.get_banner_size() > 0 {
                            if request.item[0].spec.display.instl == 1 {
                                4
                            } else {
                    ...
                    width: {
                        request.item[0].spec.display.w.or(request.item[0].spec.display.displayfmt.as_ref().and_then(|display| display.w)).unwrap_or_default()
                    },
                    height: {
                        request.item[0].spec.display.h.or(request.item[0].spec.display.displayfmt.as_ref().and_then(|display| display.h)).unwrap_or_default()
                    },
                }]
            },
            app: {
                match &request.context.app {
                    Some(app) => Some(TengmeiApp {
                        id: {
                            get_app_id(app.name.as_str())
                                .or_else(|| app.bundle.as_deref().and_then(get_app_id))
                                .or_else(|| connection.client_media_appname.as_deref().and_then(get_app_id))
                                .or_else(|| connection.client_media_apppackage.as_deref().and_then(get_app_id))
                    ...
                    None => Some(TengmeiApp {
                        id: {
                            "0".to_string()
                        },
                        name: {
                            connection.client_media_appname.clone().unwrap_or_default()
                        },
                        bundle: {
                            connection.client_media_apppackage.clone().unwrap_or_default()
                        },
                        version: {
                            "".to_string()
                        },
            ...
            device: {
                Some(TengmeiDevice {
                    ...
                    brand: {
                        request.context.device.brand.clone().or(request.context.device.make.clone())
                    },
                    model: {
                    ...
                    width: {
                        request.context.device.w.unwrap_or_default()
                    },
                    height: {
                        request.context.device.h.unwrap_or_default()
                    },
                    orientation: {
                    ...
                    density: {
                        request.context.device.pxratio.unwrap_or_default() as f32
                    },
                    ppi: {
                        request.context.device.ppi.unwrap_or_default()
                    },
                    imei: {
                    ...
                    caids: {
                        match identifiers.get_ids(513) {
                            Some(uids) => {
                                ...
                                for uid in uids {
                                    if uid.ver.is_some() {
                                        caids.push(TengmeiCaid {
                                            caid: uid.id.clone(),
                                            version: uid.ver.clone().unwrap_or_default(),
                                        });
                                    }
                                }
                                caids
                    ...
                    geo: {
                        request.context.device.geo.as_ref().and_then(|geo| {
                            if geo.lat.is_some() && geo.lon.is_some() {
                                Some(TengmeiGeo {
                                    lat: geo.lat.unwrap_or_default(),
                                    lon: geo.lon.unwrap_or_default(),
                                })
                            } else {
                                None
                            }
        ...
        let bytes = request_tengmei.encode_to_vec();

        let client = {
            let pool_tengmei_lock = pool.pool_tengmei.clone();
            let pool_tengmei = pool_tengmei_lock.read().unwrap();
            pool_tengmei.clone()
        };

        let response_tengmei_raw = client
            .post(if connection.test {
                "https://test.tengmeilab.com/methaus"
            } else {
                "https://t.tengmeilab.com/shuojun"
            })
            .body(bytes)
        ...
        let response_tengmei = match response_tengmei_raw {
            Ok(response_tengmei_raw) => {
                let status = response_tengmei_raw.status();
                if status == 204 {
                    ...
                    return Err(ResultMessage {
                        code: 992,
                        message: match response_tengmei_raw.text().await {
                ...
                match response_tengmei_raw.bytes().await {
                    Ok(bytes) => {
                        match TengmeiResponse::decode(bytes) {
                            Ok(response_tengmei) => response_tengmei,
        ...
        if response_tengmei.status_code == 1 {
            return Err(ResultMessage {
                code: 993, //defending
                message: response_tengmei.msg.clone(),
            });
        } else if response_tengmei.status_code != 0 {
            return Err(ResultMessage {
                code: 994,
                message: response_tengmei.msg.clone(),
            });
        ...
        let response = Response {
            ...
            seatbid: {
                if response_tengmei.seat_bid.len() == 0 {
                    None
                } else {
                    let mut bids = vec![];

                    for seat_bid in &response_tengmei.seat_bid {
                        for bid_tengmei in &seat_bid.bid {
                            let link_asset = LinkAsset {
                                linktype: {
                                    if bid_tengmei.app_info.as_ref().and_then(|app_info| app_info.download_url.clone()).is_some() {
                                        2
                                    } else {
                                        1
                                    }
                                ...
                                universallink: {
                                    bid_tengmei.links.as_ref().and_then(|links| links.universal_link.clone())
                                },
                                storeid: {
                                    None
                                },
                                deeplink: {
                                    bid_tengmei.links.as_ref().and_then(|links| links.deeplink.clone())
                                },
                                quickapplink: None,
                                wechatmppath: None,
                                wechatmpid: None,
                                marketurl: None,
                                downloadurl: {
                                    bid_tengmei.app_info.as_ref().and_then(|app_info| app_info.download_url.clone())
                                },
                                url: {
                                    bid_tengmei.links.as_ref().and_then(|links| links.click_url.clone()).unwrap_or_default()
                                },
                                urlfb: None,
                            };
                            ...
                            if let Some(tracking) = &bid_tengmei.tracking {
                                for url in &tracking.imp_urls {
                                    event_vec.push(Event {
                                        eventtype: 501,
                                        method: 1,
                                        url: replace_macro(url).replace("__WIN_PRICE__", &Self::encrypt_price(bid_tengmei.price, &"".to_string(), connection)),
                                        header: None,
                                        content: None,
                                    });
                                }
                                for url in &tracking.click_urls {
                                    event_vec.push(Event {
                                        eventtype: 502,
                                        method: 1,
                                ...
                                if let Some(hc) = &tracking.hc {
                                    match &hc.scheme_feedback_url {
                                    ...
                                    match &hc.click_area_report_url {
                            ...
                            bids.push(Bid {
                                ...
                                item: {
                                    bid_tengmei.imp_id.clone()
                                },
                                price: {
                                    bid_tengmei.price
                                },
                                burl: {
                                    match &bid_tengmei.tracking {
                                        Some(tracking) => {
                                            match &tracking.win_notice_url {
                                                Some(win_notice_url) => {
                                                    Some(vec![replace_macro(win_notice_url).replace("__WIN_PRICE__", "__WIN_PRICE__")])
                                                },
                                                None => None,
                                            }
                                        },
                                ...
                                lurl: {
                                    match &bid_tengmei.tracking {
                                        Some(tracking) => {
                                            match &tracking.lose_notice_url {
                                                Some(lose_notice_url) => {
                                                    Some(vec![replace_macro(lose_notice_url).replace("__LOSE_PRICE__", "__LOSE_PRICE__").replace("__WIN_PRICE__", "__LOSE_PRICE__")])
                                                },
                                                None => None,
                                            }
                                        },
                                ...
                                media: Ad {
                                    id: {
                                        bid_tengmei.creative_id.clone()
                                    },
                                    advertiser: {
                                        bid_tengmei.app_info.as_ref().and_then(|app_info| app_info.developer.clone())
                                    },
                                    advertisericon: {
                                        bid_tengmei.material.as_ref().and_then(|material| material.icon_url.clone())
                                    },
                                    display: Display {
                                        w: {
                                            bid_tengmei.material.as_ref().and_then(|material| material.width)
                                        },
                                        h: {
                                            bid_tengmei.material.as_ref().and_then(|material| material.height)
                                        },
                                        banner: {
                                            if assets.get_banner_size() > 0 {
                                                bid_tengmei.material.as_ref().and_then(|material| {
                                                    if material.video_url.is_some() {
                                                        material.video_cover_url.as_ref().map(|video_cover_url| {
                                                            Banner {
                                                                img: video_cover_url.clone(),
                                                                link: Some(link_asset.clone()),
                                                            }
                                                        })
                                                    } else {
                                                        material.image_urls.first().map(|image_url| {
                                                            Banner {
                                                                img: image_url.clone(),
                                                                link: Some(link_asset.clone()),
                                                            }
                                        ...
                                        native: {
                                            ...
                                            if let Some(material) = &bid_tengmei.material {
                                                ...
                                                if let Some(desc) = &material.desc {
                                                    asset_vec.push(Asset {
                                                ...
                                                if let Some(icon_url) = &material.icon_url {
                                                    asset_vec.push(Asset {
                                                ...
                                                if let Some(video_url) = &material.video_url {
                                                    asset_vec.push(Asset {
                                            ...
                                            if let Some(app_info) = &bid_tengmei.app_info {
                                                asset_vec.push(Asset {
                                                    ...
                                                    html: None,
                                                    app: Some(AppAsset {
                                                        name: app_info.name.clone().unwrap_or_default(),
                                                        desc: bid_tengmei.material.as_ref().and_then(|material| material.desc.clone()),
                                                        descurl: app_info.function_url.clone(),
                                                        domain: None,
                                                        bundle: app_info.bundle.clone(),
                                                        ver: app_info.version.clone(),
                                                        developer: app_info.developer.clone(),
                                                        icon: bid_tengmei.material.as_ref().and_then(|material| material.icon_url.clone()),
                                                        storeid: None,
                                                        storeurl: None,
                                                        paid: 0,
                                                        size: app_info.package_size,
                                                        md5: app_info.package_md5.clone(),
                                                        registration: None,
                                                        privacy: None,
                                                        privacyurl: app_info.privacy_policy.clone(),
                                                        permission: None,
                                                        permissionurl: app_info.permission_url.clone(),
                                                    }),
                                                });
                                            }
    ...
    async fn bidding_notify_win(url: String, win_price: i32, _next_price: i32, iv: &String, connection: &Connection, pool: &HttpPool) {
        ...
        let replaced_url = url.replace("__WIN_PRICE__", &encrypt_price);
    ...
    async fn bidding_notify_lose(url: String, lose_price: i32, _lose_reason: i32, _lose_adn_name: &String, iv: &String, connection: &Connection, pool: &HttpPool) {
        ...
        let replaced_url = url
            .replace("__LOSE_PRICE__", &encrypt_price)
            .replace("__WIN_PRICE__", &encrypt_price);
    ...
    fn encrypt_price(price: i32, _iv: &String, connection: &Connection) -> String {
        ...
        let iv_value = connection.client_tag_id.split('|').nth(2).unwrap();

        match encrypt(
            Cipher::aes_128_cbc(),
            key.as_bytes(),
            Some(iv_value.as_bytes()),
...
fn replace_macro(orig: &String) -> String {
    ...
    replaced = replaced.replace("__CTS__", "__TS_S__");
